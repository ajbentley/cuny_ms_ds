...
"typical" to mean.
    ...
    I would guess that the typical size would be around 1500. This looks like it is probably close to both the mean and median. 

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

    It would be very unusual for the samples to matc unless that student also set a seed at 23 because axiomatic of how random sampling works.
...
interval capture this value? 
    ...
    It does. While I'm not in a classroom I'm 95% confident that if I were my neighbor's interval would capture the mean, too.
...
population mean.
    ...
    95% of them should capture the mean as 95% of all samples of this data set should have a confidence interval that captures it.
...
-   Using the following function (which was downloaded with the data set), 
    ...
    2 of the 50 samples failed to contain the population mean (96%), as close as possible to the confidence level. It's not exactly the same because you can't have a fraction of a sample (you would need 2.5 sample confidence intervals to hit 95% exactly).
...
-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    ...
    For a 70% confidence interval you would set a critical value of 1.045 (ish)
...
```{r c70, eval=TRUE}
lower_vector <- samp_mean - 1.045 * samp_sd / sqrt(n) 
upper_vector <- samp_mean + 1.045 * samp_sd / sqrt(n)
c(lower_vector[1], upper_vector[1])

plot_ci(lower_vector, upper_vector, mean(population))
```

    Unsurprisingly, narrowing the confidence level increased the number of confidence intervals that do not include the mean. 14 of the 50 failed (72%).
