...
points <- namevector[seq(4, length(namevector), 3)]

pointsdf <- data.frame(points)
pointsdf$points2 <- str_extract(pointsdf$points, "\\d{5,}")
head(pointsdf)
...
pointsdf$playerpre <- sub('.* R: ', '', pointsdf$points)
pointsdf$playerpre <- sub('(.{4})(.*)', '\\1', pointsdf$playerpre)
head(pointsdf)
...
dfs <- dfs[c("playernum", "name", "state")]

dfs$points <- pointsdf$points2
dfs$playerrtg <- pointsdf$playerpre
...
write.csv(dfsr1, file = "/Users/ajbentley/Dropbox/cuny_msds/607/Project 1/chess_data.csv")
```
